              ...
              <div>4.0X</div>
						...
						<el-button v-show="item.live" class="play-speed">
							<span
								...
								@click="changeSpeed(item, index, 1)"
							>
								1.0X
							</span>
							<span
								...
								@click="changeSpeed(item, index, 2)"
							>
								2.0X
							</span>
							<span
								...
								@click="changeSpeed(item, index, 4)"
							>
								4.0X
							</span>
						</el-button>
...
export default {
	...
	mounted() {
		...
		window.addEventListener('storage', () => {
			this.videoOption.text = localStorage.getItem('userName')
				? localStorage.getItem('userName')
				: this.videoOption.text
			this.$forceUpdate()
		})
		this.videoOption.text = localStorage.getItem('userName')
			? localStorage.getItem('userName')
	...
	methods: {
		...
		async pushvideo(name, id, isReolay, starttime, endtime) {
			// 一屏播放替换url
			if (this.videoOption.size === 1) {
				...
				if (isReolay) {
					...
					this.changeLine(
						starttime,
						endtime,
						this.videoList[this.activeIndex],
						this.activeIndex,
						'picker',
						true,
						starttime,
		...
		async getVMSTOKEN(id) {
			...
			const { ipcLinkInfo } = await vmsService
				.getIpInfo(this.vmsOption.ipcLinkInfo, {
					ipcID: id
				})
				.then(res => {
					return res.data
		...
		// 回放时间线控制
		changeLine(e, item, index, type, starttime, endtime) {
			if (
				this.videoList[index].flvPlayer &&
				this.videoList[index].replaySrc &&
				this.videoList[index].flvPlayer._receivedCanPlay
			...
			if (type) {
				this.$set(this.videoList[index], 'timeKey', new Date().getTime())
				vmsService
					.getRecordList(this.ipcLinkInfo.serverIP, this.vmsOption.ipcLinkInfo, {
						devid: item.videoId,
						playtime: playtime
					})
					.then(res => {
						...
						if (!starttime) {
							this.$set(
								this.videoList[index],
								'startTime',
								new Date(hs + res.recordList[0].startTime * 1000)
							)
						}
						...
						if (!starttime) {
							...
							const playCellIndex = res.recordList.findIndex(
								element => seconds <= element.endTime && seconds >= element.startTime
							)
							this.$set(this.videoList[index], 'playCellIndex', playCellIndex)
						}
						this.videoList[index].sid = new Date().getTime()
						this.videoList[index].replaySrc =
							`http://${this.videoList[index].serverIP}:8000/playback?devid=${this.videoList[index].videoId}&sid=` +
							this.videoList[index].sid +
							'&starttime=' +
							starttime +
							'&endtime=' +
							endtime
		...
		changeSpeed(item, index, speed) {
			...
			vmsService
				.setvideorate(this.ipcLinkInfo.serverIP, this.vmsOption.ipcLinkInfo, {
					devid: item.videoId,
					scale: speed,
					sid: item.sid
				})
